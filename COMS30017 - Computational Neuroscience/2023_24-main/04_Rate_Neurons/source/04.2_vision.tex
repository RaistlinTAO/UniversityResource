...
\lhead{Computation Neuroscience - 04.2\_vision - Conor}
\rhead{\thepage}
\cfoot{}
...
\ifthenelse{\boolean{nopics}}
             {\textsl{An old drawing of the brain from underneath, the back edge is marked in red, the eyes are visible at the front, they are also marked in red, as is the optic nerve, a thick fibre heading back from the eyes and crossing about of a third the way from the front to the back of the brain, soon after they cross they merge into the rest of the brain and can't be distinguished any more.}}
             {
  \begin{center}
...
\begin{figure}
\ifthenelse{\boolean{nopics}}
             {\textsl{In this network diagram there are nodes connected by arrows, `photoreceptors' goes to `ganglion cells' which goes to `thalamus' which goes to `V1' which goes to `the rest of the visual pathway'.}}
             {
  \begin{center}
\begin{tikzpicture}
...
\begin{figure}
  \ifthenelse{\boolean{nopics}}
             {\textsl{A complicated picture showing neurons and nerves, the caption describes what's going on.}}
             {
 \begin{center}
...
\begin{figure}
  \ifthenelse{\boolean{nopics}}
             {\textsl{There is a drawing of the eye on the left of this figure with the retina marked out in angles, the blind spot is marked near 20 degrees, it is where the optic nerve leaves. The right panel is a graph with the y-axis marked `receptors per square mm' and marked from zero to 1.8x$10^5$, the x-axis is marked in degrees and there are curves for cones and rods. The cones curve peaks at zero and is tightly concentrated there, the rod curve is low near zero and highest about -20 and 20 degrees, except a big notch where the blind spot is.}}
             {
\begin{center}
...
\begin{figure}
    \ifthenelse{\boolean{nopics}}
             {\textsl{On the left a white disk is surrounded by a black circle all on a grey background, on the right is the converse, a black disk is surrounded by a white circle.}}
             {
  \begin{center}
\begin{tikzpicture}
...
\begin{figure}
    \ifthenelse{\boolean{nopics}}
             {\textsl{One the left a white oval at an angle lies along side a black oval, on the right a black oval has a white oval each side of it.}}
             {
\begin{center}
\begin{tikzpicture}
\node[rectangle,text width=3cm,text height=3cm,fill=gray](out1){};
...
\begin{figure}
    \ifthenelse{\boolean{nopics}}
             {\textsl{In a series of rows a line in shown in different locations and different orientations; beside this there is a spike train, one particular location and orientation clearly has more spikes, the further the line is from this location or orientation, the fewer spikes there are..}}
             {
\begin{center}
...
\begin{figure}
    \ifthenelse{\boolean{nopics}}
             {\textsl{There are seven illustrations, A to G, in A crosses are surrounded by triangles, in B triangles by crosses, C, D, E and F have a line of triangles surrounded by crosses, all similar except F is wider. In G a region of crosses touches a region of triangles along a line.}}
             {
\begin{center}
...
\begin{figure}
  \ifthenelse{\boolean{nopics}}
             {\textsl{Two five by five grids, on the left white in the middle, surrounded by black, surrounded by grey, on the right the two left columns are white, the three right columns are black .}}
             {
  \begin{center}
\begin{tikzpicture}
...
\begin{center}
    \ifthenelse{\boolean{nopics}}
             {\textsl{nine dots in a grid, if we number them 1 through 9 from the top left then 7, 8 and 9 are fllled black, the rest white.}}
             {
\begin{tikzpicture}
...
\begin{center}
    \ifthenelse{\boolean{nopics}}
             {\textsl{2, 5 and 8 are black. Lines jion all nine dots to three dots marked `mate', `fight' and `flee'' `mate' is black, the others white.}}
             {
\begin{tikzpicture}
\node[rectangle,rounded corners,draw=black,text width=3cm, text height=3cm](big){};
\node[circle,draw=black, fill=black](a1){};
...
\begin{center}
  \ifthenelse{\boolean{nopics}}
             {\textsl{Now the grid has a single arrow to a set of six small boxes with lines in called features and another line from those to the mate, flight and mate nodes, this second arrow is marked `learning'.}}
             {
  \begin{tikzpicture}
\node[rectangle,rounded corners,draw=black,text width=3cm, text height=3cm](big){};
\node[circle,draw=black, fill=black](a1){};
...
\begin{center}
    \ifthenelse{\boolean{nopics}}
             {\textsl{$W^1$ to $W^6$ are show each equal to a three by three grid, each grid has a row or column black, the vertical and three horizontal.}}
             {
\begin{tikzpicture}
\node at (-1,0.5){$W^1=$};
...
\begin{figure}
  \ifthenelse{\boolean{nopics}}
             {\textsl{A grid of 12 by 16 small image patches in grey shows various patterns, many of which look like little features where a dark dash is surrounded by a whiter edge..}}
             {
  \begin{center}
