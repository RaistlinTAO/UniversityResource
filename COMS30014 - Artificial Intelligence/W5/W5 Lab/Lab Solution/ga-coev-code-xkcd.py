...
# We also add the current objective fitness and relative fitness of all hosts and
#   all parasites to global lists that will be used to plot graphs later on
def write_fitness(hosts, paras, gen, file=None):
    ...
    host = hosts[len(hosts)//2] # get the median host
    para = paras[len(hosts)//2] # get the median parasite
    host["objective"] = host["solution"].count("1") # get the objective fitness
    para["objective"] = para["solution"].count("1") # get the objective fitness
    ...
    for h in hosts:  # for every host
        host_gen.append(gen)  # add the current generation to the gen list (will be used as x coord)
        host_obj.append(h["solution"].count("1"))  # add the objective fitness (y coord)..
        host_rel.append(h["fitness"])  # ..and the relative fitness (z coord) to lists
    for p in paras:  # for every parasite
        para_gen.append(gen)  # add the current generation to the gen list (will be used as x coord)
        para_obj.append(p["solution"].count("1"))  # add the objective fitness (y coord)..
...
#    there is neutrality between a solution and some of its 2-bit
#    neighbours as two bit-flips could introduce a '1' and remove
#    a '1' at the same time leaving fitness unchanged.
#
# -- 2
#
...
#    bias to be a list of A values with each value = 1/A - with a two-bit alphabet, bias = [0.5, 0.5]
...
#    then we shuffle this list so that each element i in the list points to a unique random number between 0 and N-1
# d) we pit host number i against parasite number order[i] where order[i] is now a unique random number between 0 and N-1
#    (a more naive approach would be to 1. repeatedly pick a random host and random parasite and make them fight
#    or to 2. loop through the hosts and make each host fight a randomly selected parasite
#    - what's the problem with this? some parasites will play more games than others, which will bias
#    our evaluation of parasites and introduce additional noise into evolution...)
...
def q3():
    ...
    print("3a")
    host_gen, host_obj, host_rel, para_gen, para_obj, para_rel = [], [], [], [], [], [] # initialise the data lists
    gens, best_host, best_para = do_the_ga(coevolution=False)                           # run the genetic algorithm
    print("\nDone: Evolution")
    ...
    ga_scatter(x = [host_gen, para_gen], y = [host_obj, para_obj], z = [host_rel, para_rel],
               title="3(a) Evolutionary GA Performance",
               xlabel="Generation", ylabel="Objective Fitness", zlabel="Relative Fitness",
               labels = ["Host", "Para"], ylim=[0.0, 100.0])
    ...
    print("{:4d} para: '{}' ({:.3f}) = {:3d}".format(gens,best_para["solution"],best_para["fitness"], best_para["objective"]))
    print("")
    print("Coevolution took around 200 generations to find a nearly perfect host.\n")

    ga_scatter(x = [host_gen, para_gen], y = [host_obj, para_obj], z = [host_rel, para_rel],
               title="3(b) Coevolutionary GA Performance: parasite bias "+str(para_bias)+"; lambda = "+str(para_lambda),
               xlabel="Generation", ylabel="Objective Fitness", zlabel="Relative Fitness",
               labels = ["Host", "Para"], ylim=[0.0, 100.0])
    ...
    ga_scatter(x = [host_gen, para_gen], y = [host_obj, para_obj], z = [host_rel, para_rel],
               title="3(c) Coevolutionary GA Performance: parasite bias "+str(para_bias)+"; lambda = "+str(para_lambda),
               xlabel="Generation", ylabel="Objective Fitness", zlabel="Relative Fitness",
               labels = ["Host", "Para"], ylim=[0.0, 100.0])
    ...
    ga_scatter(x = [host_gen, para_gen], y = [host_obj, para_obj], z = [host_rel, para_rel],
               title="3(d) Coevolutionary GA Performance: parasite bias "+str(para_bias)+"; lambda = "+str(para_lambda),
               xlabel="Generation", ylabel="Objective Fitness", zlabel="Relative Fitness",
               labels = ["Host", "Para"], ylim=[0.0, 100.0])
    ...
    ga_scatter(x = [host_gen, para_gen], y = [host_obj, para_obj], z = [host_rel, para_rel],
               title="3(e) Coevolutionary GA Performance: parasite bias "+str(para_bias)+"; lambda = "+str(para_lambda),
               xlabel="Generation", ylabel="Objective Fitness", zlabel="Relative Fitness",
               labels = ["Host", "Para"], ylim=[0.0, 100.0])
    ...
    ga_scatter(x = [host_gen, para_gen], y = [host_obj, para_obj], z = [host_rel, para_rel],
               title="3(f) Coevolutionary GA Performance: parasite bias "+str(para_bias)+"; lambda = "+str(para_lambda),
               xlabel="Generation", ylabel="Objective Fitness", zlabel="Relative Fitness",
               labels = ["Host", "Para"], ylim=[0.0, 100.0])
...
# See: Cartlidge, J. and Ait-Boudaoud, D. (2011). Autonomous virulence adaptation improves coevolutionary optimization.
#   IEEE Transactions on Evolutionary Computation, 15(2), 215-229.
